...
{
    public partial class PaymentTermsAddForm : Form
    {
        public PaymentTermsAddForm()
        {
            InitializeComponent();
        }
        ...
        {
            if (textPayT.Text != string.Empty && textPayT.Text != "Cash on Delivery")
            {
                try
                {
                    if (SQLConnect.Instance.ConnectState() == true)
                    {
                        List<string> result = SQLConnect.Instance.PgSQL_SELECTDataString("SELECT payterms_name FROM storagemember.payterms");
                        string name = textPayT.Text;
                        if (result.Contains(name))
                        {
                            this.LBMessageBox.Text = "This Terms has existed!";
                        ...
                        {
                            SQLConnect.Instance.PgSQL_Command("INSERT INTO storagemember.payterms(payterms_name) VALUES ('" + name + "')");
