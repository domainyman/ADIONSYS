...
namespace ADIONSYS.Plugin.POS.Shipping.Inquiry
{
    public partial class ShippingDetails : Form
    {
        ...
        public ShippingDetails(string Tp_inv)
        {
            ...
            inv_name = SQLConnect.Instance.PgSQL_SELECTDataStringsinglel("SELECT invoice FROM invoiceshipping.shippinginv WHERE ship_number='" + Tp_invoice + "'");
            ShowForm();
        }
        ...
        private void ShowForm()
        {
            try
            {
                ...
                {
                    List<string> resultDetails = SQLConnect.Instance.PgSQL_SELECTDataString("SELECT invoice,ship_details,ship_company,ship_person,ship_address,ship_tel,ship_comment,comment FROM invoiceshipping.shippinginv shippinv WHERE ship_number = '" + Tp_invoice + "'");
