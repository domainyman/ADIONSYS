...
namespace ADIONSYS.Plugin.POS.Warehose.Storage.Transfer
{
    public partial class ProductSNInsert : Form
    {
        ...
        public ProductSNInsert(int tableRow,int name_id, List<string> snlist,int sid)
        {
            sn_list = snlist;
        ...
        private List<string> insertSN()
        {
            List<string> insertlist = new List<string>();
            foreach (TextBox text in CompLayoutPanel.Controls.OfType<TextBox>())
            {
                string textStr = text.Text;
                insertlist.Add(textStr);
            }
            insertlist.RemoveAll(s => s == string.Empty);
            insertlist.RemoveAll(s => s == null);
            return insertlist;
        }

        private bool CheckInsertDataBaseSN(int product_ID, List<string> SNList)
        {
            ...
            List<string> list = SQLConnect.Instance.PgSQL_SELECTDataString("SELECT sn FROM " + hash_database );
            for (int i = 0; i < SNList.Count; i++)
            {
                if (list.Contains(SNList[i]))
                {
                    bools.Add(true);
                }
                else
        ...
        private bool CheckDuplicates(List<string> SNList)
        {
            bool hasDuplicates = SNList.GroupBy(x => x).Any(g => g.Count() > 1);
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                List<string> list_sn = insertSN();
                if (CheckDuplicates(list_sn) == false && CheckInsertDataBaseSN(product_ID, list_sn) == true)
                {                    
